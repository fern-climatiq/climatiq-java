...
@JsonDeserialize(
    using = FreightFlightsException.Deserializer.class
)
public final class FreightFlightsException extends Exception {
  private final Value value;

  private int statusCode;

  private FreightFlightsException(Value value, int statusCode) {
    this.value = value;
  ...
  public static FreightFlightsException other(Object unknownValue, int statusCode) {
    return new FreightFlightsException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "FreightFlightsException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<FreightFlightsException> {
    @Override
    public FreightFlightsException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new FreightFlightsException(value, statusCode);
    }
  }
}
