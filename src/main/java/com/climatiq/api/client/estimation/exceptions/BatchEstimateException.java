...
@JsonDeserialize(
    using = BatchEstimateException.Deserializer.class
)
public final class BatchEstimateException extends Exception {
  private final Value value;

  private int statusCode;

  private BatchEstimateException(Value value, int statusCode) {
    this.value = value;
  ...
  public static BatchEstimateException other(Object unknownValue, int statusCode) {
    return new BatchEstimateException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "BatchEstimateException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<BatchEstimateException> {
    @Override
    public BatchEstimateException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new BatchEstimateException(value, statusCode);
    }
  }
}
