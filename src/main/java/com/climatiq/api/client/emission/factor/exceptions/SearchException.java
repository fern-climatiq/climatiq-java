...
@JsonDeserialize(
    using = SearchException.Deserializer.class
)
public final class SearchException extends Exception {
  ...
  private SearchException(Value value, int statusCode) {
    this.value = value;
  ...
  public static SearchException other(Object unknownValue, int statusCode) {
    return new SearchException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "SearchException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<SearchException> {
    @Override
    public SearchException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new SearchException(value, statusCode);
    }
  }
}
