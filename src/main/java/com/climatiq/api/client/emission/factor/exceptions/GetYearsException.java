...
@JsonDeserialize(
    using = GetYearsException.Deserializer.class
)
public final class GetYearsException extends Exception {
  private final Value value;

  private int statusCode;

  private GetYearsException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetYearsException other(Object unknownValue, int statusCode) {
    return new GetYearsException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetYearsException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetYearsException> {
    @Override
    public GetYearsException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetYearsException(value, statusCode);
    }
  }
}
