...
@JsonDeserialize(
    using = GetSectorsException.Deserializer.class
)
public final class GetSectorsException extends Exception {
  private final Value value;

  private int statusCode;

  private GetSectorsException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetSectorsException other(Object unknownValue, int statusCode) {
    return new GetSectorsException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetSectorsException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetSectorsException> {
    @Override
    public GetSectorsException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetSectorsException(value, statusCode);
    }
  }
}
