...
@JsonDeserialize(
    using = GetRegionsException.Deserializer.class
)
public final class GetRegionsException extends Exception {
  private final Value value;

  private int statusCode;

  private GetRegionsException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetRegionsException other(Object unknownValue, int statusCode) {
    return new GetRegionsException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetRegionsException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetRegionsException> {
    @Override
    public GetRegionsException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetRegionsException(value, statusCode);
    }
  }
}
