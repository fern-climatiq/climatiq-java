...
@JsonDeserialize(
    using = GetSourcesException.Deserializer.class
)
public final class GetSourcesException extends Exception {
  private final Value value;

  private int statusCode;

  private GetSourcesException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetSourcesException other(Object unknownValue, int statusCode) {
    return new GetSourcesException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetSourcesException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetSourcesException> {
    @Override
    public GetSourcesException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetSourcesException(value, statusCode);
    }
  }
}
