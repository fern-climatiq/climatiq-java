...
@JsonDeserialize(
    using = GetCategoriesException.Deserializer.class
)
public final class GetCategoriesException extends Exception {
  private final Value value;

  private int statusCode;

  private GetCategoriesException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetCategoriesException other(Object unknownValue, int statusCode) {
    return new GetCategoriesException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetCategoriesException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetCategoriesException> {
    @Override
    public GetCategoriesException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetCategoriesException(value, statusCode);
    }
  }
}
