package com.climatiq.api.client.cloud.computing.exceptions;
...
@JsonDeserialize(
    using = CpuException.Deserializer.class
)
public final class CpuException extends Exception {
  private final Value value;

  private int statusCode;

  private CpuException(Value value, int statusCode) {
    this.value = value;
  ...
  public static CpuException other(Object unknownValue, int statusCode) {
    return new CpuException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "CpuException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<CpuException> {
    @Override
    public CpuException deserialize(JsonParser p, DeserializationContext ctx) throws IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new CpuException(value, statusCode);
    }
  }
}
